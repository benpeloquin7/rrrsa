...
We split by the grouping variable `grouper` and use `purrr::map_df()` which allows us to run `rsa.run_df()` over subsets of the data frame. This equivalent to subsetting `d_pragmods` by each factor in `grouper`, running `rsa.rund_df` and `rbind()`ing the results.
```{r pragmods_sim_1}
