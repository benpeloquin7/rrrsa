...
```
We can see this more clearly using data from Peloquin & Frank (under review). Here the grouping variable are different scalar familes (5 in total) named `scale`.
```{r}
...
depths <- 1:3
```
Users may also want to check only a subset of the items in their data frame for tuning. In the case of Peloquin & Frank (under review) we only wanted to tune against `entailment` pairs. The `compareItems` argument allows the user to pass a vector of items for subsetting.
```{r warning=FALSE}
