...
```
We can see this more clearly using data from Peloquin & Frank (2016). Here the grouping variable represents five different scalar familes (5 in total) named `scale`.
```{r}
...
Let's take a look at how RSA performs with best model hyper-parameters correlation = `r round(results$cor[1], 3)`, alpha = `r results$alpha[1]`, depth = `r results$depth[1]`.
```{r fig.height=5, fig.width=6}
...
## Data from `Rational speeach act models of pragmatic reasoning in reference games`

`rrrsa` includes empirical data used in Frank et al. (Under Review) model simulations. We provide users with access to the data and include a short example of running simulations.

### Description of data
```{r pragmod_head}
head(d_pragmods)
```
You'll notice some `NA` values. This is because...

The other columns represent:
--  expts
-- ...
-- 

### Example using `rrrsa` and `purrr` 

To run a basic model simulation we'll want to run individual simulations over expts. We've provided a grouping variable in the data frame (`grouper`) which allows us to to do this.

Here we split by the grouping variable `grouper` and use `purrr`'s `map_df` function which allows us to run our `rsa.run_df` over subsets of the data frame. This equivalent to subsetting `d_pragmods` by each factor in `grouper`, running `rsa.rund_df` and rbinding the results.
```{r pragmods_sim_1}
