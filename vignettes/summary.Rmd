...
In this case our grouping variable is `scales` -- we run `rsa.runDf` on each scale subset for each alpha and depth we're interested in savign the results to `res`.
...
Unlike in the previous example, we'd like to run rsa via `rsa.runDf()` over individual experiments rather than scales. We've provided a grouping variable in the data frame (`grouper`) which allows us to to do this.

Here we split by the grouping variable `grouper` and use `purrr`'s `map_df` function which allows us to run our `rsa.run_df` over subsets of the data frame. This equivalent to subsetting `d_pragmods` by each factor in `grouper`, running `rsa.rund_df` and `rbind()`ing the results.
```{r pragmods_sim_1}
