...
Rational speech act models formalize language understanding as a special case of social cognition in which `speakers` and `listeners` reason about one another recursively (Frank \& Goodman 2012; Goodman \& Stuhmuller, 2014). That is a `pragmatic listener` $P_{L_n}(m|u)$, reasons about intended meaning $m$ of an utterance $u$ by a `rational speaker`. The `rational speaker` $P_{s_n}(u|m)$ chooses an utterance $u$ given an intended meaning $m$ according to the expected utility of an utterance $U(m;u)$. In computing this utility the rational speaker reasons about how the utterance will be interpreted by a listener $P_{L_{n-1}}$. This recursion grounds out in a literal listener who interprets an utterance according to the literal semantics of the utterance $P_{L_0}(m|u)$. `alpha` $\alpha$ is a decision noise parameter. As `alpha` $\rightarrow \infty$ the speaker chooses utterance that are maximally informative.
...
`runDf` maintains all column naming and can handle multiple data types (however, the function expects numerical expressions in the `semantics` vector). For example, we can run `runDf` with a character vector for `quantity` (contrast with the factor vector used above):
```{r}
all(
