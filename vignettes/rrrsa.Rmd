...
Given the functionality for running data frames (and with multiple groups showcased above), we provide functionality for tuning `alpha` and `depth` hyperparameters. `rsa.tuneDepthAlpha` takes the same arguments one would specify for `runDf`. However the `alpha` and `depth` arguments should now be vectors of user supplied values. User should also specifiy a grouping variable if the data contains multiple sub-groups to run (such as `scales` in Peloquin & Frank, 2016).
